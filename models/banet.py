...
from models.layers import Conv1d, Res1d, Linear, LinearRes
...
class Net(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        left_graph  = boundary_gather(to_long(gpu(data["left_graph"])))
        right_graph = boundary_gather(to_long(gpu(data["right_graph"])))

        nodes, node_idcs, node_ctrs = self.map_net(graph)
        left_nodes, left_node_idcs, left_node_ctrs = self.lb_net(left_graph)
        right_nodes, right_node_idcs, right_node_ctrs = self.rb_net(right_graph)

        # actor-map fusion cycle 
        nodes        = self.a2m(nodes, graph, actors, actor_idcs, actor_ctrs)
        # try:        
        #     left_nodes   = self.a2l(left_nodes, left_graph, actors, actor_idcs, actor_ctrs)        
        #     right_nodes  = self.a2r(right_nodes, right_graph, actors, actor_idcs, actor_ctrs)       
        # except:
        #      print(data['scenario_id'])

        # fuse boundaries
        nodes  = self.l2m(nodes, graph, left_nodes, left_node_idcs, left_node_ctrs)       
        nodes  = self.r2m(nodes, graph, right_nodes, right_node_idcs, right_node_ctrs)       

        nodes  = self.m2m(nodes, graph)
        actors = self.m2a(actors, actor_idcs, actor_ctrs, nodes, node_idcs, node_ctrs)        
        actors = self.l2a(actors, actor_idcs, actor_ctrs, left_nodes, left_node_idcs, left_node_ctrs)        
        actors = self.r2a(actors, actor_idcs, actor_ctrs, right_nodes, right_node_idcs, right_node_ctrs)        
        actors = self.a2a(actors, actor_idcs, actor_ctrs)

        # Mid-Goal Net
        mid_goal = self.mid_net(actors, actor_idcs, actor_ctrs)
        mid_ctrs = [x.squeeze() for x in mid_goal]        
        actors   = self.m_m2a(actors, actor_idcs, mid_ctrs, nodes, node_idcs, node_ctrs)
        actors   = self.m_l2a(actors, actor_idcs, mid_ctrs, left_nodes, left_node_idcs, left_node_ctrs)
        actors   = self.m_r2a(actors, actor_idcs, mid_ctrs, right_nodes, right_node_idcs, right_node_ctrs)
        ...
        actors   = self.g_l2a(actors, actor_idcs, goal_ctrs, left_nodes, left_node_idcs, left_node_ctrs)
        actors   = self.g_r2a(actors, actor_idcs, goal_ctrs, right_nodes, right_node_idcs, right_node_ctrs)
...
class MidGoalNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mid_pred = nn.Sequential(LinearRes(n_actor, n_actor, norm=norm, ng=ng),
                                      nn.Linear(n_actor, 2),
        )
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> List[Tensor]:
...
class GoalNet(nn.Module):
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Dict[str, List[Tensor]]:
        ...
        reg = reg[row_idcs, sort_idcs].view(cls.size(0), cls.size(1), -1, 2)  # sorted regression 
