...
if "save_dir" not in config:
    config["save_dir"] = os.path.join(root_path, "results", "ganet")
...
class GANet(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        actors = self.m_l2a(
            actors, actor_idcs, mid_ctrs, l_nodes, l_node_idcs, l_node_ctrs
        )
        actors = self.m_r2a(
            actors, actor_idcs, mid_ctrs, r_nodes, r_node_idcs, r_node_ctrs
        )
        ...
        actors = self.g_l2a(
            actors, actor_idcs, goal_ctrs, l_nodes, l_node_idcs, l_node_ctrs
        )
        actors = self.g_r2a(
            actors, actor_idcs, goal_ctrs, r_nodes, r_node_idcs, r_node_ctrs
        )
